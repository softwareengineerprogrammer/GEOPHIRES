...
linestyles = {0: 'solid', 1: 'dashed', 2: 'dashdot', 3: 'dotted'}
...
def plot_ex(worlds, figsize=(10, 10), dpi=150, colors=colors, fontsize=10):
    ...
    for i, (title, present_per_unit) in enumerate(worlds.items()):
        ...
        axes[i].legend(wedges, labels, title=title, loc='center left', bbox_to_anchor=(0.3, 0.6, 0.0, 1))
...
def viz_wholesale(filepath, span=range(2025, 2056, 2)):
    ...
    df = pd.pivot_table(df, values='price', index=['Hour'], columns=['Year'], aggfunc=np.mean)
...
def preprocess_cambium_capacity(
    scenario,
    state,
    infl,
    base_year=2023,
    span=range(2024, 2061),
    plot=False,
    dst_dir='.',
    save=None,
    adjustment=0,
    ...
    if adjustment > 0:
        df_full[
            'capacity cost'
        ] += adjustment  # based on Fig 3-C of https://eta-publications.lbl.gov/sites/default/files/berkeley_lab_2021.11-_integrating_cambium_prices_into_electric-sector_decisions.pdf
...
def preprocess_cambium_wholesale(
    scenario,
    state,
    infl,
    daily_fat_factor=1.0,
    seasonal_fat_factor=1.0,
    base_year=2022,
    span=range(2024, 2061),
    plot=False,
    dst_dir='.',
    ...
    for i, year in enumerate(span):
        ...
        df[year] += daily_fat_factor * (df[year] - daily_price_means) + seasonal_fat_factor * (
            df[year] - seasonal_price_means
        )
    ...
    arbitrage_score = (
        df_final.groupby('Hour').mean(numeric_only=True)['price'].max()
        - df_final.groupby('Hour').mean(numeric_only=True)['price'].min()
    )  # $/MWh
    scores = {'baseload': round(baseload_score, 2), 'arbitrage': round(arbitrage_score, 2)}
    if plot:
...
def preprocess_cambium_elcc(
    scenario, state, span=range(2024, 2061), plot=False, dst_dir='../../Data/dummy', save=False
):
    ...
    df_elcc = pd.merge(pd.DataFrame({'battery_GW': range(101)}), df_elcc, on='battery_GW', how='outer').interpolate(
        method='linear'
    )

    df_full['elcc'] = df_full['battery_GW'].apply(
        lambda x: df_elcc.loc[df_elcc['battery_GW'] == int(x), 'elcc'].iloc[0]
    )
...
def compute_f(Rewaterprod, well_diam):
    ...
    f = 1.0 / np.power(-2 * np.log10(relroughness / 3.7 + 5.74 / np.power(Rewaterprod, 0.9)), 2.0)
    f = 1.0 / np.power((-2 * np.log10(relroughness / 3.7 + 2.51 / Rewaterprod / np.sqrt(f))), 2.0)
    f = 1.0 / np.power((-2 * np.log10(relroughness / 3.7 + 2.51 / Rewaterprod / np.sqrt(f))), 2.0)
    f = 1.0 / np.power((-2 * np.log10(relroughness / 3.7 + 2.51 / Rewaterprod / np.sqrt(f))), 2.0)
    f = 1.0 / np.power((-2 * np.log10(relroughness / 3.7 + 2.51 / Rewaterprod / np.sqrt(f))), 2.0)
    f = 1.0 / np.power(
        (-2 * np.log10(relroughness / 3.7 + 2.51 / Rewaterprod / np.sqrt(f))), 2.0
    )  # 6 iterations to converge

    # if laminar
    f = np.where(Rewaterprod < 2300.0, 64.0 / nonzero(Rewaterprod), f)
...
def viscositywater(Twater):
    ...
    muwater = 2.414e-5 * np.power(
        10, 247.8 / (Twater + 273.15 - 140)
...
def vaporpressurewater(Twater):
    ...
    return np.where(
        Twater < 100,
        133.322 * (10 ** (8.07131 - 1730.63 / (233.426 + Twater))) / 1000,
        133.322 * (10 ** (8.14019 - 1810.94 / (244.485 + Twater))) / 1000,
    )


def compute_drilling_cost(
    well_tvd, well_diam, lateral_length=0, numberoflaterals=1, total_drilling_length=None, drilling_cost=None
):
    ...
    if well_diam > 0.3:  # units in meters
        if lateral_length > 0:
            return (0.2553 * well_md**2 + 1716.7157 * well_md + 500867.0) * 1e-6 / 1.15
        else:
            return (0.2818 * well_tvd**2 + 1275.5213 * well_tvd + 632315.0) * 1e-6 / 1.15
    else:
        if lateral_length > 0:
            return (0.2898 * well_md**2 + 822.1507 * well_md + 680563.0) * 1e-6 / 1.15
        else:
            return (0.3021 * well_tvd**2 + 584.9112 * well_tvd + 751368.0) * 1e-6 / 1.15
...
def compute_latlon_distance(lat1, lon1, lat2, lon2):
    ...
    return (
        np.arccos(np.sin(lat1) * np.sin(lat2) + np.cos(lat1) * np.cos(lat2) * np.cos(lon2 - lon1)) * 6371
    )  # kilometers
...
def latlon_tres_to_depth(
    df_maps, query_northing, query_easting, tres, MJth_per_km3=5.1e9, thickness=500, eta=0.1, L=25
):
    ...
    for depth in all_depths:
        ...
        row = df_temp.iloc[
            [
                (
                    np.sqrt((query_northing - df_temp['Northing']) ** 2 + (query_easting - df_temp['Easting']) ** 2)
                ).argmin()
            ]
        ]
        temp = row['T'].values[0]
...
def latlon_depth_to_tres(
    df_maps, query_northing, query_easting, well_depth, MJth_per_km3=5.1e9, thickness=1000, eta=0.1, L=25
):
    """Get depth required to reach a target temperature at a latlon location."""
    ...
    ref_row = df_temp.iloc[
        [(np.sqrt((query_northing - df_temp['Northing']).abs() + (query_easting - df_temp['Easting']).abs())).argmin()]
    ]
    ...
    for depth in depths:
        ...
        row = df_temp.iloc[
            [
                (
                    np.sqrt((query_northing - df_temp['Northing']).abs() + (query_easting - df_temp['Easting']).abs())
                ).argmin()
            ]
        ]
        temp = row['T'].values[0]
...
def retrieve_weather(lat, lon, year, station_idx_limit=10):
    ...
    while len(data) != 8760:
        ...
        data = Hourly(
            station, start - timedelta(days=2), end + timedelta(days=2)
        )  # get more days to account for timezone shifting
        data = data.fetch()
