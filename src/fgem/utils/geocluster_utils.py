...
class data:
    def __init__(self, fname, case, fluid):
        ...
        with h5py.File(fname, 'r') as file:
            ...
            self.kWe_avg = self.We * self.GWhr / (1000.0 * self.time[-1] * 86400.0 * 365.0)
            self.kWt_avg = self.Wt * self.GWhr / (1000.0 * self.time[-1] * 86400.0 * 365.0)
    ...
    def interp_outlet_states(self, point):
        ...
        points = list(
            iter.product(
                (point[0],), (point[1],), (point[2],), (point[3],), (point[4],), (point[5],), (point[6],), self.time
            )
        )
    ...
    def interp_kWe_avg(self, point):
        ivars = self.ivars[:-1]
        return self.GWhr * interpn(ivars, self.We, point) / (1000.0 * self.time[-1] * 86400.0 * 365.0)

    def interp_kWt_avg(self, point):
        ivars = self.ivars[:-1]
        return self.GWhr * interpn(ivars, self.Wt, point) / (1000.0 * self.time[-1] * 86400.0 * 365.0)
...
class TEA:
    ...
    def verify(self):  # Verify inputs are within allowable bounds
        ...
        if self.Drilling_cost_per_m < 0 or self.Drilling_cost_per_m > 10000:
            print(
                'Error: Drilling costs per m of measured depth must be between 0 and 10,000 $/m. Simulation terminated.'
            )
            self.error = 1
        if self.O_and_M_cost_plant < 0 or self.O_and_M_cost_plant > 0.2:
            print(
                'Error: Operation & maintance cost of surface plant (expressed as fraction of total surface plant capital cost) must be between 0 and 0.2. Simulation terminated.'
            )
            self.error = 1
        ...
        if self.End_use == 1:
            if self.Direct_use_heat_cost_per_kWth < 0 or self.Direct_use_heat_cost_per_kWth > 10000:
                print(
                    'Error: Capital cost for direct-use surface plant must be between 0 and 10,000 $/kWth. Simulation terminated.'
                )
                self.error = 1
            if self.Electricity_rate < 0 or self.Electricity_rate > 0.5:
                print(
                    'Error: Electricity rate in direct-use for pumping power must be between 0 and 0.5 $/kWh. Simulation terminated.'
                )
                self.error = 1
        if self.End_use == 2:
        ...
        if self.Fluid == 2 and self.End_use == 2:
            ...
            if self.Pre_Cooling_Delta_T < 0 or self.Pre_Cooling_Delta_T > 15:
                print(
                    'Error: CO2 temperature decline after turbine and before compressor must be between 0 and 15 degrees C. Simulation terminated.'
                )
                self.error = 1
    ...
    def initialize(self):
        ...
        self.numberofcases = (
            len(self.FlowRateVector)
            * len(self.HorizontalLengthVector)
            * len(self.DepthVector)
            * len(self.GradientVector)
            * len(self.DiameterVector)
            * len(self.TinVector)
            * len(self.KrockVector)
        )

        self.Time_array = np.linspace(
            0, self.Lifetime * 365 * 24 * 3600, 1 + self.Lifetime * self.Number_of_points_per_year
        )  # [s]
        ...
        self.Utilization_efficiency_correlation_temperatures = np.array(
            [100, 200, 385]
        ...
        self.Utilization_efficiency_correlation_conversion = np.array(
            [0.2, 0.45, 0.45]
        )  # Efficiency of ORC conversion from production exergy to electricity based on GEOPHIRES correlation [-]
        self.Heat_to_power_efficiency_correlation_temperatures = np.array(
            [100, 200, 385]
        )  # Linear correlation based on Chad Augustine's thesis [deg.C] plus plateaued above 200 deg. C
        self.Heat_to_power_efficiency_correlation_conversion = np.array(
            [0.05, 0.14, 0.14]
        )  # Conversion from enthalpy to electricity [-]

        # Calculate dead-state enthalpy and entropy in case of electricity production
        if self.End_use == 2:
            self.h_0 = interpn((self.Pvector, self.Tvector), self.enthalpy, np.array([self.P0, self.T0]))[
                0
            ]  # dead-state enthalpy [J/kg]
            self.s_0 = interpn((self.Pvector, self.Tvector), self.entropy, np.array([self.P0, self.T0]))[
                0
            ]  # dead-state entropy [J/kg/K]
        ...
        if self.End_use == 2:  # electricity generation
            self.Instantaneous_exergy_production = np.zeros(
                len(self.Time_array)
            )  # Produced exergy only (independent from injection conditions)
            self.Instantaneous_exergy_extraction = np.zeros(
                len(self.Time_array)
            )  # Difference between produced exergy and injected exergy
            self.Instantaneous_electricity_production_method_1 = np.zeros(
                len(self.Time_array)
            )  # based on exergy produced (only for water)
            self.Instantaneous_electricity_production_method_2 = np.zeros(
                len(self.Time_array)
            )  # based on exergy extracted
            self.Instantaneous_electricity_production_method_3 = np.zeros(
                len(self.Time_array)
            )  # based on thermal efficiency
            self.Instantaneous_electricity_production_method_4 = np.zeros(
                len(self.Time_array)
            )  # based on direct turbine expansion (for CO2)
            self.Instantaneous_utilization_efficiency_method_1 = np.zeros(
                len(self.Time_array)
            )  # conversion from produced exergy to electricity
            self.Instantaneous_utilization_efficiency_method_2 = np.zeros(
                len(self.Time_array)
            )  # conversion from extracted exergy to electricity
            self.Instantaneous_themal_efficiency = np.zeros(
                len(self.Time_array)
            )  # conversion from enthalpy to electricity
            self.Annual_electricity_production = np.zeros(self.Lifetime)
        if self.Fluid == 2:
            self.Instantaneous_turbine_power = np.zeros(
                len(self.Time_array)
    ...
    def calculateLC(self):
        ...
        if min(self.Linear_production_temperature) > self.T_in:
            ...
            Discount_vector = 1.0 / np.power(1 + self.Discount_rate, np.linspace(0, self.Lifetime - 1, self.Lifetime))
            # if self.End_use == 1:   #direct-use heating
            self.LCOH = (
                (self.TotalCAPEX + np.sum(self.OPEX_Plant * Discount_vector))
                * 1e6
                / np.sum(self.Annual_heat_production / 1e3 * Discount_vector)
            )  # $/MWh
            self.LCOH100 = (
                (self.TotalCAPEX100 + np.sum(self.OPEX_Plant * Discount_vector))
                * 1e6
                / np.sum(self.Annual_heat_production / 1e3 * Discount_vector)
            )  # $/MWh
            self.LCOH500 = (
                (self.TotalCAPEX500 + np.sum(self.OPEX_Plant * Discount_vector))
                * 1e6
                / np.sum(self.Annual_heat_production / 1e3 * Discount_vector)
            )  # $/MWh
            self.LCOH1500 = (
                (self.TotalCAPEX1500 + np.sum(self.OPEX_Plant * Discount_vector))
                * 1e6
                / np.sum(self.Annual_heat_production / 1e3 * Discount_vector)
            )  # $/MWh
            if self.LCOH < 0:
            ...
            if self.Average_electricity_production == 0:
                ...
                self.LCOE = (
                    (self.TotalCAPEX + np.sum(self.OPEX_Plant * Discount_vector))
                    * 1e6
                    / np.sum((self.Annual_electricity_production - self.Annual_pumping_power) / 1e3 * Discount_vector)
                )  # $/MWh
                self.LCOE100 = (
                    (self.TotalCAPEX100 + np.sum(self.OPEX_Plant * Discount_vector))
                    * 1e6
    ...
    def calculatedrillinglength(self):
        if self.Configuration == 1:
            self.Drilling_length = (
                self.Hor_length_user + 2 * self.Depth_user
            )  # Total drilling depth of both wells and lateral in U-loop [m]
        elif self.Configuration == 2:
            self.Drilling_length = (
                self.Hor_length_user + self.Depth_user
    ...
    def calculateheatproduction(self):
        ...
        self.Average_fluid_density = interpn(
            (self.Pvector, self.Tvector),
            self.density,
            np.dstack(
                (
                    0.5 * self.P_in + 0.5 * self.Linear_production_pressure,
                    0.5 * self.T_in + 0.5 * self.Linear_production_temperature + 273.15,
                )
            )[0],
        )
        self.hprod = interpn(
        ...
        self.Instantaneous_heat_production = (
            self.Flow_rate * (self.hprod - self.hinj) / 1000
        )  # Heat production based on produced minus injected enthalpy [kW]

        # Calculate annual heat production (kWh)
        self.Annual_heat_production = (
            8760
            / 5
            * (
                self.Instantaneous_heat_production[0::4][0:-1]
    ...
    def calculateelectricityproduction(self):
        ...
        self.s_inj = interpn(
            (self.Pvector, self.Tvector), self.entropy, np.array([self.P_in, self.T_in + 273.15])
        )  # injected entropy [J/kg/K]

        self.Instantaneous_exergy_production = (
            self.Flow_rate * (self.h_prod - self.h_0 - self.T0 * (self.s_prod - self.s_0))
        ) / 1000  # [kW]
        self.Instantaneous_exergy_extraction = (
            self.Flow_rate * (self.h_prod - self.h_inj - self.T0 * (self.s_prod - self.s_inj))
        ) / 1000  # [kW]
        ...
        if self.Fluid == 1:
            ...
            if (
                self.T_in >= 50
                and min(self.Linear_production_temperature) >= 100
                and max(self.Linear_production_temperature) <= 385
            ):
                self.Instantaneous_utilization_efficiency_method_1 = np.interp(
                    self.Linear_production_temperature,
                ...
                self.Instantaneous_electricity_production_method_1 = (
                    self.Instantaneous_exergy_production * self.Instantaneous_utilization_efficiency_method_1
                )  # [kW]
                self.Instantaneous_themal_efficiency = np.interp(
                    self.Linear_production_temperature,
                    self.Heat_to_power_efficiency_correlation_temperatures,
                    self.Heat_to_power_efficiency_correlation_conversion,
                    left=0,
                )  # Utilization efficiency based on conversion of produced exergy to electricity
                self.Instantaneous_electricity_production_method_3 = (
                    self.Instantaneous_heat_production * self.Instantaneous_themal_efficiency
                )  # [kW]
            ...
            self.Annual_electricity_production = (
                8760
                / 5
                * (
            ...
            h_turbine_out_ideal = interpn(
                (self.Pvector_ap, self.svector_ap),
                self.hPs,
                np.dstack((np.ones(self.TNOP) * self.Turbine_outlet_pressure * 1e5, self.s_prod))[0],
            )
            self.Instantaneous_turbine_power = (
                self.Flow_rate * (self.h_prod - h_turbine_out_ideal) * self.Turbine_isentropic_efficiency / 1000
            )  # Turbine output [kW]
            h_turbine_out_actual = (
                self.h_prod - self.Instantaneous_turbine_power / self.Flow_rate * 1000
            )  # Actual fluid enthalpy at turbine outlet [J/kg]
            self.T_turbine_out_actual = (
                interpn(
                    (self.Pvector_ap, self.hvector_ap),
                    self.TPh,
                    np.dstack((np.ones(self.TNOP) * self.Turbine_outlet_pressure * 1e5, h_turbine_out_actual))[0],
                )
                - 273.15
            )
            ...
            if min(self.T_turbine_out_actual) > 37 and self.T_in > 32:
                ...
                while valuefound == 0:
                    # print('b')
                    Pre_compressor_h = interpn(
                        (self.Pvector, self.Tvector),
                        self.enthalpy,
                        np.array([self.Turbine_outlet_pressure * 1e5, self.Pre_cooling_temperature + 273.15]),
                    )

                    Pre_cooling = (
                        self.Flow_rate * (h_turbine_out_actual - Pre_compressor_h) / 1e3
                    )  # Pre-compressor cooling [kWth]
                    Pre_compressor_s = interpn(
                        (self.Pvector, self.Tvector),
                        self.entropy,
                        np.array([self.Turbine_outlet_pressure * 1e5, self.Pre_cooling_temperature + 273.15]),
                    )

                    Post_compressor_h_ideal = interpn(
                        (self.Pvector_ap, self.svector_ap), self.hPs, np.array([self.P_in, Pre_compressor_s[0]])
                    )
                    Post_compressor_h_actual = (
                        Pre_compressor_h
                        + (Post_compressor_h_ideal - Pre_compressor_h) / self.Compressor_isentropic_efficiency
                    )  # Actual fluid enthalpy at compressor outlet [J/kg]
                    self.Post_compressor_T_actual = (
                        interpn(
                            (self.Pvector_ap, self.hvector_ap),
                            self.TPh,
                            np.array([self.P_in, Post_compressor_h_actual[0]]),
                        )
                        - 273.15
                    )
                    Compressor_Work = self.Flow_rate * (Post_compressor_h_actual - Pre_compressor_h) / 1e3  # [kWe]
                    Post_cooling = (
                        self.Flow_rate * (Post_compressor_h_actual - self.h_inj) / 1e3
                ...
                T_air_out_pre_cooler = (
                    self.T_turbine_out_actual + self.Pre_cooling_temperature
                ) / 2  # Air outlet temperature in pre-cooler [deg.C]
                cp_air = np.interp(
                    0.5 * T_air_in_pre_cooler + 0.5 * T_air_out_pre_cooler, self.Tair_for_cp_array, self.cp_air_array
                )  # Air specific heat capacity in pre-cooler [J/kg/K]
                m_air_pre_cooler = (
                    Pre_cooling * 1000 / (cp_air * (T_air_out_pre_cooler - T_air_in_pre_cooler))
                )  # Air flow rate in pre-cooler [kg/s]

                T_air_in_post_cooler = self.T0 - 273.15
                T_air_out_post_cooler = (
                    self.Post_compressor_T_actual + self.T_in
                ) / 2  # Air outlet temperature in post-cooler [deg.C]
                cp_air = np.interp(
                    0.5 * T_air_in_post_cooler + 0.5 * T_air_out_post_cooler, self.Tair_for_cp_array, self.cp_air_array
                )  # Air specific heat capacity in post-cooler [J/kg/K]

                m_air_post_cooler = (
                    Post_cooling * 1000 / (cp_air * (T_air_out_post_cooler - T_air_in_post_cooler))
                )  # Air flow rate in post-cooler [kg/s]

                Air_cooling_power = (
                    m_air_pre_cooler + m_air_post_cooler
                ) * 0.25  # Electricity for air-cooling, assuming 0.25 kWe per kg/s [kWe]
                # print(self.Instantaneous_turbine_power)
                # print(Compressor_Work)
                self.Instantaneous_electricity_production_method_4 = (
                    self.Instantaneous_turbine_power * self.Generator_efficiency
                    - Compressor_Work
                    - Air_cooling_power
                    - ResistiveHeating
                )  # Net electricity using CO2 direct turbine expansion cycle [kWe]
                ...
                self.AveInstElectricityProduction = np.average(
                    self.Instantaneous_electricity_production_method_4
                )  # [kW]
                # check if negative
                if min(self.Instantaneous_electricity_production_method_4) < 0:
                    self.error_codes = np.append(
                        self.error_codes, 5500
                    )  # Calculated electricity generation is negative
    ...
    def calculatepumpingpower(self):
        ...
        self.PumpingPower = (
            (self.P_in - self.Linear_production_pressure)
            * self.Flow_rate
            / self.Average_fluid_density
            / self.Pump_efficiency
            / 1e3
        )  # Pumping power [kW]
        self.PumpingPower[self.PumpingPower < 0] = (
            0  # Set negative values to zero (if the production pressure is above the injection pressure, we throttle the fluid)
        )
        self.Annual_pumping_power = (
            8760
            / 5
            * (
                self.PumpingPower[0::4][0:-1]
                + self.PumpingPower[1::4]
                + self.PumpingPower[2::4]
                + self.PumpingPower[3::4]
                + self.PumpingPower[4::4]
            )
        )  # kWh
    ...
    def calculatecapex(self):
        ...
        if self.End_use == 1:  # direct-use heating
            self.CAPEX_Surface_Plant = (
                np.max(self.Instantaneous_heat_production) * self.Direct_use_heat_cost_per_kWth / 1e6
            )  # [M$]
        elif self.End_use == 2:  # electricity production
            if self.Fluid == 1:
                self.CAPEX_Surface_Plant = (
                    np.max(self.Instantaneous_electricity_production_method_1) * self.Power_plant_cost_per_kWe / 1e6
                )  # [M$]
            elif self.Fluid == 2:
                self.CAPEX_Surface_Plant = (
                    np.max(self.Instantaneous_electricity_production_method_4) * self.Power_plant_cost_per_kWe / 1e6
                )  # [M$]

        self.TotalCAPEX = (
            self.CAPEX_Drilling + self.CAPEX_Surface_Plant
        )  # Total system capital cost (only includes drilling and surface plant cost) [M$]
        self.TotalCAPEX100 = (
            self.Drilling_length * 100 / 1e6 + self.CAPEX_Surface_Plant
        )  # Total system capital cost (only includes drilling and surface plant cost) [M$]
        self.TotalCAPEX500 = (
            self.Drilling_length * 500 / 1e6 + self.CAPEX_Surface_Plant
        )  # Total system capital cost (only includes drilling and surface plant cost) [M$]
        self.TotalCAPEX1500 = (
            self.Drilling_length * 1500 / 1e6 + self.CAPEX_Surface_Plant
        )  # Total system capital cost (only includes drilling and surface plant cost) [M$]
    ...
    def calculatopex(self):
        ...
        if self.End_use == 1:  # direct-use heating
            self.OPEX_Plant = (
                self.O_and_M_cost_plant * self.CAPEX_Surface_Plant
                + self.Annual_pumping_power * self.Electricity_rate / 1e6
            )  # Annual plant O&M cost [M$/year]
        elif self.End_use == 2:  # electricity production
    ...
    def printresults(self):
        ...
        # Print conditions
        print('Flow rate = ' + f'{self.Flow_user:.1f}' + ' kg/s')
        print('Lateral Length = ' + str(round(self.Hor_length_user)) + ' m')
        print('Vertical Depth = ' + str(round(self.Depth_user)) + ' m')
        print('Geothermal Gradient = ' + f'{self.Gradient_user*1000:.1f}' + ' deg.C/km')
        print('Wellbore Diameter = ' + f'{self.Diameter_user:.4f}' + ' m')
        print('Injection Temperature = ' + f'{self.Tin_user-273.15:.1f}' + ' deg.C')
        print('Thermal Conductivity = ' + f'{self.krock_user:.2f}' + ' W/m/K')
        ...
        if len(self.error_codes) > 0:
            ...
            if np.in1d(2000, self.error_codes):  # plot the temperature and pressure for these
                print(
                    'Error: Water injection temperature and/or production tempeature fall outside the range of the ORC correlations. These correlations require injection temperature larger than 50 deg.C and production temperature in the range of 100 to 385 deg.C. Electricity production set to 0. \n'
                )

            if np.in1d(
                3000, self.error_codes
            ):  # CO2 injection temperature cannot be below 32 degrees C (CO2 must be supercritical)
            ...
            if np.in1d(4000, self.error_codes):  # Turbine outlet CO2 temperature dropped below 37 degrees C
                print(
                    'Error: Too low CO2 turbine outlet temperature. Turbine outlet temperature must be above 37 degrees C.\n'
                )
        ...
        if self.End_use == 1:
            ...
            print('Average Production Temperature = ' + f'{self.AveProductionTemperature:.1f}' + ' deg.C')
            print('Average Production Pressure = ' + f'{self.AveProductionPressure:.1f}' + ' bar')
            if np.in1d(1000, self.error_codes) == False:
                print('Average Heat Production = ' + f'{self.AveInstHeatProduction:.1f}' + ' kWth')
                print('First Year Heat Production = ' + f'{self.FirstYearHeatProduction/1e3:.1f}' + ' MWh')
                print(' ')
                print('### Cost Results ###')
                print('Total CAPEX = ' + f'{self.TotalCAPEX:.1f}' + ' M$')
                print('Drilling Cost = ' + f'{self.CAPEX_Drilling:.1f}' + ' M$')
                print('Surface Plant Cost = ' + f'{self.CAPEX_Surface_Plant:.1f}' + ' M$')
                print('OPEX = ' + f'{self.AverageOPEX_Plant*1000:.1f}' + ' k$/year')
                print('LCOH = ' + f'{self.LCOH:.1f}' + ' $/MWh')
        ...
        if self.End_use == 2:
            ...
            if np.in1d(1000, self.error_codes) == False:
                print('Average Heat Production = ' + f'{self.AveInstHeatProduction:.1f}' + ' kWth')
                print('Average Net Electricity Production = ' + f'{self.AveInstNetElectricityProduction:.1f}' + ' kWe')
                print('First Year Heat Production = ' + f'{self.FirstYearHeatProduction/1e3:.1f}' + ' MWh')
                print(
                    'First Year Electricity Production = ' + f'{self.FirstYearElectricityProduction/1e3:.1f}' + ' MWh'
                )
                print(' ')
                print('### Cost Results ###')
                print('Total CAPEX = ' + f'{self.TotalCAPEX:.1f}' + ' M$')
    ...
    def plotresults(self):
        ...
        if self.End_use == 2:
            ...
            if (
                self.Fluid == 2
                and np.in1d(3000, self.error_codes) == False
                and np.in1d(4000, self.error_codes) == False
            ):
                ...
                turbineoutletS = interpn(
                    (self.Pvector, self.Tvector),
                    self.entropy,
                    np.array([self.Turbine_outlet_pressure * 1e5, turbineoutletT + 273.15]),
                )[0]

                # Ts at compressor inlet
                compressorinletT = self.Pre_cooling_temperature
                compressorinletS = interpn(
                    (self.Pvector, self.Tvector),
                    self.entropy,
                    np.array([self.Turbine_outlet_pressure * 1e5, compressorinletT + 273.15]),
                )[0]

                # Ts at compressor outlet
                compressoroutletT = self.Post_compressor_T_actual[0]
                compressoroutletS = interpn(
                    (self.Pvector, self.Tvector), self.entropy, np.array([self.P_in, compressoroutletT + 273.15])
                )[0]

                # Ts at re-injection
                reinjectionT = self.T_in
                reinjectionS = interpn(
                    (self.Pvector, self.Tvector), self.entropy, np.array([self.P_in, reinjectionT + 273.15])
                )[0]

                cycleTarray = np.array(
                    [turbinletT, turbineoutletT, compressorinletT, compressoroutletT, reinjectionT, turbinletT]
                )
                cycleSarray = np.array(
                    [turbinletS, turbineoutletS, compressorinletS, compressoroutletS, reinjectionS, turbinletS]
                )
        ...
        for i in range(len(parameter1range)):
            ...
            for j in range(len(parameter2range)):
                print(
                    ...
                    + str(j + 1 + i * len(parameter2range))
                    + '/'
                    + str(len(parameter1range) * len(parameter2range))
                )
                if parameter2index == 0:
    ...
    def optimalLCOH(self):
        ...
        avekwthrange = np.zeros(
            [
                len(flowratevectorforoptimum),
                len(horizontallengthvectorforoptimum),
                len(depthvectorforoptimum),
                len(tinvectorforoptimum),
            ]
        )
        lcoerange = np.zeros(
            [
    ...
    def optimalLCOH2(self):
        ...
        # create zero vectors to store results
        avekwthrange = np.zeros(
            [len(flowratevectorforoptimum), len(horizontallengthvectorforoptimum), len(tinvectorforoptimum)]
        )
        lcoerange = np.zeros(
            [len(flowratevectorforoptimum), len(horizontallengthvectorforoptimum), len(tinvectorforoptimum)]
        )
