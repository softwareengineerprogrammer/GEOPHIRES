...
class BasePowerPlant:
    ...
    def __init__(self, ppc, Tres, cf, timestep=timedelta(hours=1)):
    ...
    def compute_thermalexergy(self, T, T_amb):
        ...
        thermal_exergy = (
            (
                (A - B * T_amb_k) * (T_k - T_amb_k)
                + (B - C * T_amb_k) / 2.0 * (T_k**2 - T_amb_k**2)
                + C / 3.0 * (T_k**3 - T_amb_k**3)
                - A * T_amb_k * np.log(T_k / T_amb_k)
            )
            * 2.2046
            / 947.83
        )  # MJ/kg
...
class GEOPHIRESORCPowerPlant(BasePowerPlant):
    ...
    def compute_cplant(self, MaxProducedTemperature, min_cost=0):
        ...
        Cplantcorrelation = (
            max(min_cost, CCAPP1 * math.pow(self.powerplant_capacity / 15.0, -0.06))
            * 1e-6
            * self.powerplant_capacity
            * 1e3
        )  # $MM
    ...
    def compute_geofluid_consumption(self, T, T_amb, m_turbine):
        ...
        if T_amb < 15.0:
            ...
            D1 = 2.713e-3
            D0 = -9.1841e-2
            Tfraction = (T_amb - 5.0) / 10.0
        else:
            C1 = 2.713e-3
            C0 = -9.1841e-2
            D1 = 2.676e-3
            D0 = -1.012e-1
            Tfraction = (T_amb - 15.0) / 10.0
        etaull = C1 * T + C0
        etauul = D1 * T + D0
    ...
    def compute_injection_temp(self, T, T_amb, m_turbine):
        ...
        reinjtll = C1 * T + C0
        reinjtul = D1 * T + D0
        Tinj = max((1.0 - Tfraction) * reinjtll + Tfraction * reinjtul, 0.0)  # deg C
...
class GEOPHIRESFlashPowerPlant(BasePowerPlant):
    ...
    def compute_cplant(self, MaxProducedTemperature, min_cost=0):
        ...
        a = CCAPPRL / PRL**b
        Cplantcorrelation = (
            max(min_cost, 0.8 * a * math.pow(self.powerplant_capacity, b)) * self.powerplant_capacity * 1000.0 / 1e6
    ...
    def compute_geofluid_consumption(self, T, T_amb, m_turbine):
        ...
        self.etau = (1.0 - Tfraction) * etaull + Tfraction * etauul
    ...
    def compute_injection_temp(self, T, T_amb, m_turbine):
        ...
        Tinj = max((1.0 - Tfraction) * reinjtll + Tfraction * reinjtul, 0.0)
...
class HighEnthalpyCLGWGPowerPlant(BasePowerPlant):
    ...
    def __init__(self, Tres, Tamb, m_prd, num_prd, cf=1.0, high_resolution=False, k=2):
        ...
        self.k = k
        self.model = CustomUnpickler(
            open(os.path.join(parent_path, 'data/powerplants', 'HighEnthalpyCLGWGPowerPlant.pkl'), 'rb')
        ).load()
...
class ORCPowerPlant(BasePowerPlant):
    ...
    def __init__(self, Tres, Tamb, m_prd, num_prd=None, ppc=None, cf=1.0, k=2):
        ...
        self.k = k
        self.model = CustomUnpickler(
            open(os.path.join(parent_path, 'data/powerplants', 'ORCPowerPlant.pkl'), 'rb')
        ).load()
        ...
        self.inv_distances = (
            1 / (self.distances + 1e-3) ** 2
        )  # important choice on how much to interpolate ... more impactful when simualted data is sparse ...
